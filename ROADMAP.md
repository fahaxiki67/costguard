...
v0.1.14 在 v0.1.13 的实际使用与 Windows 兼容基线上进一步强化 A/B/C 独立性、逐 Sheet
逐行覆盖证明、公式/筛选/合并/隐藏结构和 `.xls` 能力边界，同时保留空白启动、资料
文件/文件夹与拖拽入口、解析失败持久化、统一字体、受控知识资产、项目版本链和历史
综合单价提示。以下生产门槛仍未全部关闭；在关闭前只能标记为预览/Beta 候选，不能标记为正式生产版。
