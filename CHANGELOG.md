...
### 证据门控修复（fix/v0127-evidence-gates，基线核查 C1–C5/C7）

- 聚合归组键纳入归一特征与归一单位（`aggregate.group_key_of`）：同名同码
  不同单位（如商砼 m² 对 m³）分行累计并显式标记不可比，绝不跨单位求和；
  同名变体（空格/标点差异）同单位仍合并。差异表同步同口径拆分，并新增
  "期间不连续"断点——单位拆分或缺失期造成的空档绝不跨期取"上期值"，
  多单位逻辑清单的独立单位桶在差异列可见标注"分行列示、不可比"。
- 清单匹配 fuzzy_name：名称相似 ≥97 但归一单位不同的合并组改判
  `incomparable`（禁止合并），组内保留双单位证据；相似度 85–97 的两个
  高概率组留下"疑似相关"提示，不再静默。
- 镜像比较：两侧单位不同时数量/单价/金额差值一律 `None` 并标注
  "不可比（两侧单位不同）"，不再输出无业务意义的数值差。
- 结算导入：隐藏/veryHidden 工作表即使表头规范也不得自动确认，转
  pending 待人工确认角色；不写入期次与明细，原始网格保留在保真层。
- 发布工具链：Windows 构建脚本改用 LF 生成 SHA256SUMS.txt；校验单
  拒收含 CRLF 行尾的校验和文件（`read_bytes` 检查，fail-closed）。
- UI 措辞：运行级"结果不可用"不再显示为"数据库不可写"；fail-closed
  具体原因经 reason 后缀透出。
...
确认生命周期与控制规则预发行（schema v51）：合同事实确认生命周期（v48）、
PDF 逐页人工对照复核（v49）、对上控制基准候选与五态上限比较（v50）、框架/管理性
协议费率规则（v51，多百分比候选抽取 + 公式型候选 + 基数人工设定 + Decimal 试算，
真实框架协议语料实测验证）。真实 OCR 质量、费率基数自动关联、报告导出接入、
Office 真机、50k/200k 性能、签名与公证仍为 `PENDING / NOT VERIFIED`。

### 框架/管理性协议费率规则（schema v51）

- 迁移 v51：rate_rules（比例/基数类型/基数说明/税口径/上限下限/状态/审计）。
- 费率抽取：触发词命中的句子按句拆分，句内每个百分比各成一条候选
  （3%/全角％/3 个点/百分之三），同句去重；公式型比例（如"按结算金额的（9-X）%"）
  产出"需人工解读"候选，绝不静默丢弃；真实框架协议语料实测验证。
- 确认必须人工设定基数类型与说明（"按结算价3%"≠"按不含税价3%"）；
  公式型候选确认时必须人工填写比例；候选未确认不得试算。
- 试算为 Decimal 确定性计算（round2），支持上限封顶/下限兜底；
  全部流转写审计 Evidence。
- 工作台"费率规则（框架协议）…"对话框：扫描候选/确认/拒绝/试算。
...
预发行候选修正版：逐页 PDF/OCR 边界、离线 RapidOCR 模型完整性校验、资料重新分类审计和
...
本版把 v0.1.16 之后已经完成并验证的改动整理为可下载的阶段性预发行候选；真实
工程黄金案例、四环境 Office、完整大规模现场、签名公证和 Windows 修复后原生
构建仍保持未完成状态。

### Changed

- 启动页不再自动扫描默认工作空间或加载旧项目/演示项目；用户必须明确点击
  “刷新”或“打开已有项目…”，也可以直接选择资料或拖拽资料开始新项目。
- 解析入口增加源工作簿 Sheet 目录/范围盘点、Sheet 索引与原始网格预校验；
  解析物化和覆盖证明使用外层事务，失败时回滚半成品业务数据并保留失败
  批次与 Evidence。
- 源工作簿盘点进一步保留有值单元格数量、物理行数量和坐标指纹，能在
  Sheet 尺寸未变时发现解析器漏掉整行/整段单元格；导入失败提示保留核心层
  的具体原因并明确提示用户可检查后重试。
- 正式分析入口在首次运行前物化当前导入的逐清单聚合行，修复新项目直接点击
  “双向校核/异常检测”时因缺少 `period_totals` 回写行而无法进入分析的问题；
  不改变 A/B/C 的独立性和 fail-closed 门槛。
- 发布清单进一步收紧性能报告结构、黄金登记表和资产路径校验，避免空结果、
  畸形阶段或合成案例被误读为生产通过。
- 新增只读匹配 Benchmark 评价器：只接受人工确认的稳定项目项身份，分别输出
  precision、recall、F1、false positive/negative 及置信档位计数；缺少真值为
  `PENDING`，标签结构无法对齐为 `INCOMPARABLE`，不改变现有匹配算法。
- Windows 安装包隐私审计在启动时统一使用 UTF-8 输出，避免 GitHub Windows
  runner 的 cp1252 控制台因中文提示崩溃；本修复仍需 Windows 当前版本现场重跑确认。
- 性能基准的取消/异常终态现在同时记录规模、阶段、原因和时间；发布清单拒绝
  空结果、缺步骤或缺终止证据的现场，取消仍只显示为条件状态，不会被误报为通过。

### Validation boundary

- 当前工作树全量自动化回归为 742 项，Ruff 和差异检查通过；这只证明代码路径，
  不替代真实工程资料验收。
- 当前工作树已重新构建 Apple Silicon 本地测试 DMG；首页空白和原生 Mac 文件选择器
  冒烟检查通过。该包为 ad-hoc 签名、未公证测试包，不代表正式发布。
- 脱敏真实黄金案例仍为 0；WPS/Excel 四环境、1万/5万/20万行完整现场性能、
  签名/公证和异常退出恢复仍保持 PENDING。
- 匹配 Benchmark 已具备可重复的机器 JSON 与 Markdown 输出，但当前没有经过
  授权、脱敏并人工标注的真实项目项真值，不能据此宣称匹配 precision/recall 已通过。
- 本版发布前未接入外部移动存储中的测试资料；未连接、未读取、未复制任何未授权的
  外部资料，待后续明确授权和脱敏样本后再接入。
