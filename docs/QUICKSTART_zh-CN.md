...
>
> **首次创建项目提示"没有权限"（macOS 文件夹权限，B1）**：macOS 会限制
> 未公证应用写入"文稿"文件夹。若创建项目时提示没有权限/无法创建目录：
> 打开 **系统设置 → 隐私与安全性 → 文件与文件夹**，找到 **Jiadun（价盾）**
> 并打开"文稿文件夹"开关；或直接在 **完全磁盘访问权限** 中添加 Jiadun.app
> （一次授权最省事）。授权后重启价盾即可。改用自定义工作空间（home 顶层
> 等非"文稿"目录）也可避开该限制。注意：直接编辑 settings.json 调整
> 工作空间时，必须**先完全退出价盾**再改——应用退出时会用内存中的配置
> 覆盖同名文件，运行中改会被静默还原。
>
> 验证 DMG 完整性（可选）：终端执行
