...
> 当前状态：**v0.1.25 预发行/预览候选**。七步主链和 Mac 图形界面已可运行，并提供
> **未公证、ad-hoc 本地签名的 macOS DMG**（Apple Silicon 原生），同时提供未签名的 Windows x64 安装包与便携包。本版建立合同事实确认生命周期（抽取条款一律候选、
> 人工确认才是已确认事实、推翻须留理由、拒绝条款退出运行契约、工作台条款确认对话框）、
> OCR 合同的逐页人工对照复核、对上控制基准候选与五态上限比较、以及框架/管理性协议
> 费率规则（多百分比候选抽取、基数必须人工设定、Decimal 确定性试算），
> 并保留逐页混合 PDF 提取边界（页级 1..N 覆盖、缺页/错序/失败页 fail-closed、
> 离线 RapidOCR 模型清单校验、OCR 元数据进入 Evidence）、资料中心分类/方向/状态
> 可追溯导入、后台导入与 fail-closed 历史合同门控，以及备份恢复 fail-closed、
> Windows 工作簿所有权、A/B/C 独立性、原始范围覆盖、公式/筛选/合并/隐藏结构和
> `.xls` 能力边界，还有空白启动、文件/文件夹与拖拽导入、解析失败 Evidence、统一字体、
> 差异雷达、镜像匹配、受控规则和知识库、项目版本链及历史综合单价提示。
> 真实扫描件 OCR 质量、扫描页逐条对照复核、真实 Microsoft Excel/WPS、四种 Office 环境、
> 真实黄金案例、现场性能、签名和公证仍未完成，暂不标记为正式生产版。
> 详见[发布说明](docs/RELEASE_NOTES_v0.1.24.md)和
...
4. 首次实际使用时，点击 **「导入资料文件…」** 或 **「导入资料文件夹…」**，
   也可以把资料直接拖到首页；价盾会先创建项目，再把结算表和合同/纪要文件作为只读副本导入。
   如果只是想体验完整流程，再明确点击 **「体验匿名演示」**，三分钟走完
   导入 → 标准化 → Decimal 金额校核 → 同义匹配 → 对上对下差异 → 异常检测 →
   Excel/Word 导出（全程使用完全合成的匿名演示数据）。
...
性能基准成功后默认清理可再生的合成现场。若报告要作为发布证据或交给外部复核，
必须追加 `--keep-workspace`，保留输入文件、数据库和 XLSX 导出文件，才能复核
文件哈希和 ZIP 结构。

性能脚本会在每次运行的唯一目录下逐阶段报告生成、导入、分页、搜索、异常、
匹配、聚合、校核和可选导出；失败或取消的阶段保留现场，不会被记为通过。目标机器的实测基准、四种
Office 环境、签名公证、迁移恢复和真实案例回归仍属于 v0.1.25 发布门槛。
