...
> 当前状态：**v0.1.18 预发行/预览候选**。七步主链和 Mac 图形界面已可运行，并提供
> **未公证、ad-hoc 本地签名的 macOS DMG**（Apple Silicon 原生），同时提供未签名的 Windows x64 安装包与便携包。本版进一步收紧
> A/B/C 独立性、原始范围覆盖、公式/筛选/合并/隐藏结构和 `.xls` 能力边界，同时保留
...
> 详见[发布说明](docs/RELEASE_NOTES_v0.1.18.md)和
...
Office 环境、签名公证、迁移恢复和真实案例回归仍属于 v0.1.18 发布门槛。
