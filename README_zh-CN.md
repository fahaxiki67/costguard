...
> 当前状态：**v0.1.13 预览候选**。七步主链和 Mac 图形界面已可运行，并提供
> **未公证、ad-hoc 本地签名的 macOS DMG**（Apple Silicon 原生）。本版增加空白启动、
> 文件/文件夹与拖拽导入、解析失败 Evidence 和统一字体，同时保留不可变 Run Contract、
> 逐 Sheet 覆盖证明、四级 fail-closed 状态、差异雷达、镜像匹配、
