"""Windows 移植补丁的验收测试（v0.1.5）。

覆盖三项移植：
1. generate_demo_data：zip 条目 create_system 固定为 3（Unix）——Windows 上
   zipfile 默认 create_system=0，生成字节与 macOS/仓库不一致，确定性测试必挂；
2. header_detect：小计词补"本页小计/本页合计"（支付报表按页小计行）；
3. workbench.SheetConfirmDialog：被门控工作表的人工确认闭环（候选预填、
...
class TestDemoZipDeterminism:
    def test_demo_archive_entries_are_unix_create_system(self):
        """演示数据的 zip 条目 create_system 必须固定为 3（跨平台字节一致）。"""
        for name in ("演示-对上结算-第1至3期.xlsx", "演示-合同摘录-合成.docx"):
            path = DEMO_DIR / name
            assert path.is_file(), f"缺少演示文件 {name}"
            with zipfile.ZipFile(path) as zf:
                for info in zf.infolist():
                    assert info.create_system == 3, (
                        f"{name} 内条目 {info.filename} create_system="
                        f"{info.create_system}（Windows 默认 0 会导致跨平台字节不一致）"
                    )

    def test_generator_normalization_sets_create_system(self):
        """生成器的 _normalize_zip 写出的条目必须带 create_system=3。"""
        import tempfile
