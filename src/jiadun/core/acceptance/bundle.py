...
def _tracked_diff_hash(root: Path) -> str:
    # 以 HEAD 为基准读取完整 tracked patch，覆盖已暂存、未暂存和混合状态；
    # 仅调用 git 只读接口，路径过滤避免私密目录进入运行包。未跟踪文件仍
    # 通过 repository_state.untracked_files 单独记录，不伪装成 tracked diff。
    raw = _git(root, "diff", "HEAD", "--binary", "--", ".", ":!local_private_data")
...
def build_acceptance_bundle(
    *,
    ...
    matching_file = root / "src/jiadun/core/matching/matching.py"
    golden_input = {
        "missing": None,
        "decimal": "10.00",
    ...
    bundle: dict[str, Any] = {
        ...
        "golden_vector": {
            # 协议标识已随 v0.1.8 对外使用；品牌重命名不得改动黄金向量身份。
