...
def _settings() -> QSettings:
    """返回价盾当前设置命名空间；所有新值只写入这里。"""
    return QSettings(branding.ORGANIZATION_NAME, branding.PRODUCT_SLUG)


def _legacy_settings() -> QSettings:
    """返回旧 CostGuard 设置命名空间，仅用于只读兼容。"""
    return QSettings(branding.LEGACY_PRODUCT_NAME, branding.LEGACY_PRODUCT_NAME)


def _setting_value(key: str, default=None):
    """读取新设置，缺少时回读旧设置并复制该项到新命名空间。

    迁移只涉及窗口偏好和最近打开时间，不触碰项目数据库或原始资料；旧
    QSettings 保留不删，便于回滚和审计。
    """
    current = _settings()
    value = current.value(key)
    if value is not None:
        return value
    legacy = _legacy_settings().value(key)
    if legacy is None:
        return default
    current.setValue(key, legacy)
    current.sync()
    return legacy
...
def _project_error_text(error: Exception) -> str:
    ...
    if text.startswith(("not a Jiadun project", "not a CostGuard project")):
        return "所选目录不是价盾项目，请选择包含项目数据库的目录。"
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"{branding.PRODUCT_DISPLAY_FULL} — 工程经营合规智能工作台")
        icon = load_app_icon()
        if icon is not None:
            self.setWindowIcon(icon)
        geo = _setting_value(_GEOMETRY_KEY)
        if geo is not None:
    ...
    def _projects_page(self) -> QWidget:
        ...
        title = QLabel(branding.PRODUCT_DISPLAY_NAME)
        title.setStyleSheet(
